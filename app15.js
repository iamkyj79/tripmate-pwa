...
async function regenerateTripAi() {
  ...
  try {
    ...
    const contextNote = [
      ...
      '매일 가능한 시간대에 식사 일정을 반드시 1개 이상 포함하고, 온전한 여행일에는 점심과 저녁을 모두 포함한다. 첫날과 마지막날도 항공편 시간에 맞춰 아침·점심·저녁 중 가능한 식사를 절대 누락하지 않는다.',
      '각 관광·식사 일정에는 주변 실명 맛집을 최소 3곳 포함하고, 주요 관광 일정에는 주변 쇼핑 장소 2~3곳과 추천 품목·이동수단·소요시간·거리를 포함한다.',
    ].filter(Boolean).join('\n');
