...
const ATTRACTION_GUIDES = [
  ...
  {match:/자금성|고궁|forbidden city|palace museum/i,eyebrow:'BEIJING · WORLD HERITAGE',title:'자금성(고궁박물원)',image:'https://images.unsplash.com/photo-1584646098378-0874589d76b1?auto=format&fit=crop&w=1400&q=85',summary:'명·청 왕조의 황궁으로, 거대한 궁궐 축과 전각을 따라 중국 황실 건축과 생활 문화를 살펴볼 수 있습니다.',points:['오문에서 신무문 방향으로 이동하는 일방향 관람 동선을 기준으로 계획하면 편합니다.','태화전·중화전·보화전은 중심축의 핵심 전각입니다.','전각뿐 아니라 동서쪽 궁원과 보물관까지 보려면 충분한 관람 시간을 확보하세요.'],tip:'입장 정책과 운영시간은 변동될 수 있으므로 방문 전 공식 안내와 예약 상태를 확인하세요.'},
...
function guideForItem(item) {
  ...
  const found = ATTRACTION_GUIDES.find(guide => guide.match.test(text));
...
window.openAttractionGuide = function(id) {
  ...
  $('scheduleModalBody').innerHTML = `<article class="place-guide"><div class="place-guide-hero"><img src="${e(guide.image)}" alt="${e(guide.title)} 여행 이미지"><div class="place-guide-overlay"><span>${e(guide.eyebrow)}</span><h2>${e(guide.title)}</h2><p>${e(item.place || trip.destination || '')}</p></div></div><div class="place-guide-feed"><section class="guide-lead"><span class="guide-badge">여행 안내</span><h3>${e(guide.summary)}</h3></section><section class="guide-story"><div class="guide-story-index">01</div><div><b>놓치지 말아야 할 포인트</b><ul>${guide.points.map(point => `<li>${e(point)}</li>`).join('')}</ul></div></section><section class="guide-story"><div class="guide-story-index">02</div><div><b>일정에 적용할 팁</b><p>${e(guide.tip)}</p></div></section><section class="guide-story guide-live"><div class="guide-story-index">✓</div><div><b>방문 전 최종 확인</b><p>운영시간, 휴관일, 예약 및 입장 조건은 변동될 수 있습니다. 방문 직전에 공식 채널에서 확인해 주세요.</p></div></section></div></article>`;
