...
const ATTRACTION_GUIDES = [
  ...
  {match:/유니버[설셜]|universal|环球度假区/i,eyebrow:'BEIJING · THEME PARK',title:'유니버설 베이징 리조트',image:'https://upload.wikimedia.org/wikipedia/commons/c/c6/Beijing_universal_entrance.jpg',summary:'영화와 캐릭터 세계관을 바탕으로 어트랙션·공연·테마 공간을 즐기는 대형 리조트입니다.',points:['인기 어트랙션은 오전에 우선 배치하면 대기시간 관리에 유리합니다.','공연 시간과 이동 거리를 고려해 구역별로 묶어 관람하세요.','시티워크의 식사와 쇼핑 시간을 별도로 확보하면 일정이 여유롭습니다.'],tip:'운영시간·휴장 시설·입장 조건은 방문일 공식 앱 또는 홈페이지에서 확인하세요.'},
  {match:/이화원|summer palace|颐和园|頤和園/i,eyebrow:'BEIJING · GARDEN & LAKE',title:'이화원',image:'https://upload.wikimedia.org/wikipedia/commons/d/db/Longevity_Hill_of_the_Summer_Palace.jpg',summary:'쿤밍호와 만수산을 중심으로 조성된 황실 정원으로 자연 경관과 누각·회랑이 조화를 이룹니다.',points:['장랑과 불향각, 쿤밍호를 연결해 보면 대표 경관을 효율적으로 볼 수 있습니다.','호수 주변 이동 거리가 길어 핵심 관람 지점을 먼저 정하는 편이 좋습니다.','날씨가 좋은 날에는 호수와 산의 원경을 함께 담기 좋습니다.'],tip:'전체 관람에는 시간이 많이 필요하므로 다음 일정과 이동시간을 넉넉히 잡으세요.'},
  {match:/만리장성|무톈위|모전욕|mutianyu|慕田峪|great wall/i,eyebrow:'BEIJING · GREAT WALL',title:'무톈위 만리장성',image:'https://upload.wikimedia.org/wikipedia/commons/f/fa/Great_Wall_of_China_July_2006.JPG',summary:'베이징 화이러우구에 있는 만리장성 구간으로, 능선을 따라 이어지는 성벽과 망루가 잘 보존된 대표 관광지입니다.',points:['성벽 위 보행 구간은 경사와 계단이 많으므로 편한 신발을 준비하세요.','케이블카·리프트 등산과 도보 구간을 체력과 관람시간에 맞춰 선택하세요.','귀환 교통편과 탑승 장소를 미리 확인하면 일정을 안정적으로 운영할 수 있습니다.'],tip:'운영시간과 케이블카 운행 여부는 날씨에 따라 달라질 수 있으므로 방문 직전에 확인하세요.'},
];

function attractionGuideCandidate(item) {
  const text = `${item.item_type || ''} ${item.title || ''} ${item.place || ''}`;
  // A meal or hotel action can be combined with an attraction in one itinerary row.
  // Known attraction names therefore take precedence over the generic exclusions.
  if (ATTRACTION_GUIDES.some(guide => guide.match.test(text) && !(guide.exclude && guide.exclude.test(text)))) return true;
