...
async function renderIt() {
  ...
  $('tabbody').innerHTML = '<div id="iform"></div>' + rows.map(item => {
    ...
    return `<div class="item"><div class="row"><div><b>Day ${item.day_no} · ${(item.start_time || '').slice(0, 5)} · ${e(item.title)}</b><div class="muted">${e(item.place || '')}</div></div><div class="actions">${typeof attractionGuideCandidate === 'function' && attractionGuideCandidate(item) ? `<button class="small guide-button" onclick="openAttractionGuide('${item.id}')">안내</button>` : ''}<button class="small" onclick="memoI('${item.id}')">메모</button><button class="small" onclick="editI('${item.id}')">수정</button><button class="small danger" onclick="delI('${item.id}')">삭제</button></div></div>${route}<div class="chips">${item.estimated_cost != null ? `<span class="chip">예상 이용금액 ${e(item.estimated_cost)} ${e(item.currency || '')}</span>` : ''}${item.meal_type ? `<span class="chip">🍽 ${e(item.meal_type)}</span>` : ''}</div>${item.user_memo ? `<div class="memobox"><b>📝 내 메모</b><div>${e(item.user_memo)}</div></div>` : ''}${restaurantBlock}${shoppingBlock}</div>`;
  }).join('');
  window._i = rows;
  if (typeof repairZeroMovementRows === 'function') repairZeroMovementRows(rows);
}
