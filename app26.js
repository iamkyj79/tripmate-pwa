// v52: render each daily dashboard directly onto exactly one A4 page.
(function(){
  ...
  function pdfBackdrop(){
    const raw=typeof homeTripImage==='function'?homeTripImage(trip):'https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1600&q=80';
    const image=String(raw||'').replace(/["'()]/g,'');
    return `<div aria-hidden="true" style="position:absolute;inset:0;background:linear-gradient(rgba(255,255,255,.92),rgba(255,255,255,.965)),url('${image}') center/cover no-repeat;z-index:0"></div>`;
  }
  async function renderA4Pages(host,filename){
    document.body.appendChild(host);
    try{
      if(document.fonts?.ready)await document.fonts.ready;
      const pageNodes=[...host.querySelectorAll('.pdf-export-page')];
      if(!pageNodes.length)throw Error('PDF로 출력할 페이지가 없습니다.');
      const {jsPDF}=window.jspdf;
      const pdf=new jsPDF({unit:'mm',format:'a4',orientation:'portrait',compress:true});
      for(let index=0;index<pageNodes.length;index+=1){
        const canvas=await html2canvas(pageNodes[index],{scale:2,useCORS:true,allowTaint:false,backgroundColor:'#ffffff',logging:false,width:794,height:1122,windowWidth:794,scrollX:0,scrollY:0});
    ...
    try{
      ...
      days.forEach((day,dayIndex)=>{
        ...
        chunks.forEach((chunk,chunkIndex)=>{
          ...
          const mapHeight=first?500:620;
          pages.push(`<section class="pdf-export-page" style="position:relative;width:794px;height:1122px;box-sizing:border-box;background:#fff;overflow:hidden">${pdfBackdrop()}<div style="position:relative;z-index:1;height:100%;box-sizing:border-box">${first?pdfHero():''}<div style="padding:${first?'16':'22'}px 24px 14px">${first?pdfSummary(distance,places):''}${pdfDayHeader(day,chunkIndex+1,chunks.length)}<div style="display:grid;grid-template-columns:1.12fr .88fr;gap:14px;align-items:start"><div class="pdf-keep"><div style="display:flex;justify-content:space-between;margin-bottom:8px"><b style="font-size:15px;color:#123a72">지도 & 동선</b><span style="font-size:9px;color:#64748b">Day ${day} 일정 순서</span></div><div style="height:${mapHeight}px;border:1px solid #dfe9f6;border-radius:18px;overflow:hidden;background:rgba(255,255,255,.88)">${routeSvg(all.filter(x=>x.item_type!=='flight'))}</div></div><div><div style="display:flex;justify-content:space-between;margin-bottom:8px"><b style="font-size:15px;color:#123a72">일정 타임라인</b><span style="font-size:9px;color:#64748b">${chunks.length>1?'계속':''}</span></div>${timelineHtml(chunk)}</div></div>${last?pdfTips():''}${pdfFooter()}</div></div></section>`);
        });
      });
      if(!pages.length)pages.push(`<section class="pdf-export-page" style="position:relative;width:794px;height:1122px;background:#fff;overflow:hidden">${pdfBackdrop()}<div style="position:relative;z-index:1">${pdfHero()}<div style="padding:24px">${pdfSummary(distance,places)}<div style="padding:80px;text-align:center;color:#64748b">등록된 일정이 없습니다.</div>${pdfFooter()}</div></div></section>`);
      const host=document.createElement('div');host.style='position:fixed;left:-10000px;top:0;font-family:Arial,"Noto Sans KR",sans-serif;color:#172b4d;background:#fff;width:794px;padding:0;margin:0;z-index:-1';host.innerHTML=pages.join('');
      await renderA4Pages(host,`${trip.title}_${includeActual?'여행이력':'여행계획'}_대시보드.pdf`);
